...
var helpers = {
	...
	updateImages: function (imdb, callback) {
		...
		console.log("Fetching images for show: " + imdb);
				...
				if (!err && data) {
					...
					Show.find({
						...
						Show.update({
									...
									$set: {
										images: {
											poster: ((data.images.poster.full == null || data.images.poster.full == "") ? "https://upload.wikimedia.org/wikipedia/commons/6/6c/Popcorn_Time_logo.png" : data.images.poster.full),
											fanart: ((data.images.fanart.full == null || data.images.fanart.full == "") ? "https://upload.wikimedia.org/wikipedia/commons/6/6c/Popcorn_Time_logo.png" : data.images.fanart.full),
											banner: ((data.images.banner.full == null || data.images.banner.full == "") ? "https://upload.wikimedia.org/wikipedia/commons/6/6c/Popcorn_Time_logo.png" : data.images.banner.full),
										},
									}
								},
								function(err, show) {
					...
					console.error("Couldn't fetch images for '" + imdb + "' error: " + err)
				}
	...
	refreshDatabase: function() {
		...
		async.each(providers, function(provider, cb) {
			...
			async.mapLimit(allShows[0], 2, helpers.extractTrakt, function(err, results) {
				...
				config.lastRefresh = Math.floor((new Date).getTime()/1000);
			});
		});
	},
