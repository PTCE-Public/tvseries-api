...
exports.getShowDetails = function(data, cb) {
    if(cb == null) return;
    if(!config.extractMoreInfo) return cb(true, null);

    var showDetails = [];
    request(BASE_URL + SHOWS + data.id + "/"+ data.slug +"/", function(err, res, html) {
        if(err) return cb(err, null);

        var $ = cheerio.load(html);

        try {
            var imdb = $("div[itemtype='http://schema.org/AggregateRating']").children('a').first().attr('href').match(/\/title\/(.*)\//)[1];
        } catch (err){
            console.log("IMDB code missing for " + data.show);
            return cb(err, null);
        }

        showDetails.push({imdb: imdb, html: html});

        return cb(null, showDetails);
    });
};
